...
public class LinearLayerNetwork {
	...
	protected void changeTheWeights(ArrayList<Double> ins, double learnRate, double momentum) {
		...
		//Double for loop - 1st loop goes through each neuron, 2nd loop goes through each weight
		//Counters: nct - neurals counter, ict - inputs counter, wct - weights counter
		
		for (int nct=0; nct<numNeurons; nct++) {
			double theIn;
			for (int ict = 0; ict < ins.size() + 1; ict++) {
				int wct = ict + nct*(numWeights/numNeurons);		//Calculate weights counter
				//If bias input, set it to 1.0
				if (ict == 0) theIn = 1.0; else theIn = ins.get(ict-1);
				//Calculate changeInWeights
				changeInWeights.set(wct, theIn*deltas.get(nct)*learnRate +
			 		changeInWeights.get(wct) * momentum);
				//Calculate weights
				weights.set(wct, weights.get(wct ) +
			 			changeInWeights.get(wct));
			}
		}
	}
	...
	public ArrayList<Double> weightedDeltas() {
		...
		//For loop to repeat number of inputs
		for(int inputCount=0; inputCount<numInputs; inputCount++) {
			//Initialising sum variable store weighted sum
			double sumDelta = 0;
			//Loop to go through each neuron
			for (int neuronCount=0; neuronCount<numNeurons; neuronCount++) {
				//Calculate counter for weights
				int deltaCount =neuronCount * (numInputs+1) + inputCount +1;
				
				//Multiply deltas with weights, add to sumDelta
				sumDelta += deltas.get(neuronCount) * weights.get(deltaCount);
			}
			//Add weighted delta to the array list
	...
	public String getWeights() {
		...
		for (int i=0; i<weights.size(); i++) {
			...
			s+=String.format("%.5f", weights.get(i)) + " ";
		}
